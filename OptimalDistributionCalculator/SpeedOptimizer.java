// Same as optimizer but prioritizes reaching a specific speed first
...
public class SpeedOptimizer extends Optimizer {
    ...
    // Set the target speed you want to reach
    private double targetSpeed = 134;
    ...
    public SpeedOptimizer(DamageCalculator calculator) {
        super(calculator);
        player = new Player();
            ...
            for (int j = 0; j < feetlist.size(); j++) {
                for (int k = 0; k < spherelist.size(); k++) {
                    for (int l = 0; l < ropelist.size(); l++) {
                        ...
                        if (damage > highestDamage && totalSpeed >= targetSpeed) {
    ...
    public double MaxDamage(Relic[] set) {
        ...
        for (int i = 0; i < addRolls * set.length; i++) {
            ...
            double damage7;
            double totalSpeed = (player.baseSpeed * ((player.speedUp/100)+1)) + player.flatSpeed;
            if (totalSpeed < targetSpeed) {
                damage7 = 999999999;
            } else {
                player.flatSpeed += rollSpeed;
                damage7 = calculator.AverageDamage(player);
                player.flatSpeed -= rollSpeed;
            }
